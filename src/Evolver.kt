 ...
 */
class Evolver(private val assets: List<String>, private val selector: Selector,
    ...
    init {
        if (popSize < 1 || portfolioSize < 1 ||
                (terminateThreshold != null && terminateThreshold < 0)) {
            throw IllegalArgumentException(
                    "population size, portfolio size, and termination threshold must be non-zero")
        } else if (portfolioSize > assets.size) {
            throw IllegalArgumentException(
                    "portfolio size must be less than number of available assets")
        } else if (iterations == null && terminateThreshold == null) {
            throw IllegalArgumentException(
                    "must provide # of iterations or a termination threshold, or both")
        }
    }

    private fun namePortfolio(portfolio: Portfolio): List<Pair<String, Double>> {
        return portfolio.allocations.map { allocation: Allocation ->
            Pair(this.assets[allocation.asset], allocation.amount)
        }
    }
